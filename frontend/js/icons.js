const HERO_PATHS = {
  ...
  "tag": `<path stroke-linecap="round" stroke-linejoin="round" d="M9.568 3H5.25A2.25 2.25 0 0 0 3 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 0 0 5.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 0 0 9.568 3Z"/>
  ...
  "play-circle-solid": `<path fill-rule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12Zm14.024-.983a1.125 1.125 0 0 1 0 1.966l-5.603 3.113A1.125 1.125 0 0 1 9 15.113V8.887c0-.857.921-1.4 1.671-.983l5.603 3.113Z" clip-rule="evenodd"/>`,
  "arrow-right-on-rectangle": `<path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-6a2.25 2.25 0 0 0-2.25 2.25v13.5A2.25 2.25 0 0 0 7.5 21h6a2.25 2.25 0 0 0 2.25-2.25V15M12 9l3 3m0 0-3 3m3-3H3"/>`
};

/* Eski "lucide" nom -> Heroicons nom moslashtiruvi (HTML/JS kodini o'zgartirmasdan
   icon kutubxonasini almashtirish uchun). Ba'zi ikonlar bevosita bir xil nomga ega. */
const LUCIDE_TO_HERO = {
  "activity": "chart-bar",
  ...
  "clapperboard": "film",
  "clock": "clock",
  "combine": "squares-plus",
  "copy": "document-duplicate",
  "eye": "eye",
  "file-video": "video-camera",
  "film": "film",
  "hard-drive": "server",
  ...
  "loader": "arrow-path",
  "message-circle": "chat-bubble-left",
  "plus": "plus",
  "refresh-cw": "arrow-path",
  "save": "check",
  "search": "magnifying-glass",
  "settings": "cog-6-tooth",
  "settings-2": "adjustments-horizontal",
  ...
  "check": "check",
  "log-out": "arrow-right-on-rectangle",
};

/* Kichik to'ldirilgan status nuqtasi ("circle" — lucide'da doiracha edi) uchun
   svg emas, oddiy <span> ishlatamiz — chunki heroicons'da mos "solid dot" yo'q. */
function _renderDot(el) {
  const span = document.createElement("span");
  span.className = "inline-block rounded-full bg-current";
  span.style.width = "7px";
  span.style.height = "7px";
  const style = el.getAttribute("style");
  if (style) span.setAttribute("style", span.getAttribute("style") + ";" + style.replace(/width:[^;]+;?|height:[^;]+;?/g, ""));
  if (el.className) span.className += " " + el.className;
  return span;
}

/**
 * Sahifadagi barcha [data-lucide] elementlarni Heroicons SVG bilan almashtiradi.
 * `window.lucide.createIcons()` bilan bir xil nomga ega (drop-in almashtiruv),
 * shu bilan mavjud HTML/JS kodini o'zgartirmasdan icon kutubxonasi almashtiriladi.
 */
function renderHeroIcons(root) {
  const scope = root || document;
  const nodes = scope.querySelectorAll("[data-lucide]");
  nodes.forEach((el) => {
    const name = el.getAttribute("data-lucide");
    if (name === "circle") {
      el.replaceWith(_renderDot(el));
      return;
    }
    const heroName = LUCIDE_TO_HERO[name] || name;
    const pathContent = HERO_PATHS[heroName];
    if (!pathContent) return; // noma'lum icon — jim o'tkazib yuboriladi

    const svgNs = "http://www.w3.org/2000/svg";
    const svg = document.createElementNS(svgNs, "svg");
    svg.setAttribute("xmlns", svgNs);
    svg.setAttribute("viewBox", "0 0 24 24");
    svg.setAttribute("fill", heroName === "play-circle" ? "currentColor" : "none");
    svg.setAttribute("stroke", heroName === "play-circle" ? "none" : "currentColor");
    svg.setAttribute("stroke-width", "1.5");
    svg.innerHTML = pathContent;

    // Eski elementdagi class/style ni ko'chiramiz
...
/* Lucide CDN skriptining o'rnini bosuvchi shim — mavjud sahifalar
   `if (window.lucide) lucide.createIcons();` kodini o'zgartirmasdan ishlaydi. */
window.lucide = { createIcons: () => renderHeroIcons(document) };
